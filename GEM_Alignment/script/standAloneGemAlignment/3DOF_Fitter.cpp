...
int main() {
  ...
  const char* input_name = "/eos/user/t/toakhter/tamu_mual/2025/Run2025B_muon0_ZMu_150X_dataRun3_Prompt_v1.root";
  ...
  const char* outname_prefix = "Run2025B_muon0_ZMu_150X_dataRun3_Prompt_v1";
    ...
    for (int j = -1; j < 2; j = j + 2){             // Region loop
      for (int i = 0; i<36;i++){                    // Chamber loop
        for (int k = 1; k < max_layer; k++){        // Layer loop
          ...
          if (tt_tmp->GetEntries()<10){continue;}
